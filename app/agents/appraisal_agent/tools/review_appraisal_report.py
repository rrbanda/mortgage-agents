...
@tool
def review_appraisal_report(application_data: dict) -> str:
    """
    Basic review appraisal report functionality.
    
    This tool provides basic appraisal report review.
    ...
    """
    try:
        # Extract basic data from flexible dict
        ...
        report = [
            'REVIEW APPRAISAL REPORT ANALYSIS REPORT',
            '=' * 50,
        ...
        logger.error(f'Error during review_appraisal_report: {e}')
        return f'Error during review_appraisal_report: {str(e)}'
...
def validate_tool() -> bool:
    ...
    try:
        ...
        result = review_appraisal_report.invoke({'application_data': test_data})
        return 'ANALYSIS REPORT' in result and 'ARCHITECTURE' in result
        
    except Exception as e:
        print(f'review_appraisal_report tool validation failed: {e}')
        return False