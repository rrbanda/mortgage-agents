...
@tool
def find_comparable_sales(application_data: dict) -> str:
    """
    Basic find comparable sales functionality.
    
    This tool provides basic comparable sales analysis.
    ...
    try:
        ...
        report = [
            'FIND COMPARABLE SALES ANALYSIS REPORT',
            '=' * 50,
        ...
        logger.error(f'Error during find_comparable_sales: {e}')
        return f'Error during find_comparable_sales: {str(e)}'
...
def validate_tool() -> bool:
    ...
    try:
        ...
        result = find_comparable_sales.invoke({'application_data': test_data})
        return 'ANALYSIS REPORT' in result and 'ARCHITECTURE' in result
        
    except Exception as e:
        print(f'find_comparable_sales tool validation failed: {e}')
        return False